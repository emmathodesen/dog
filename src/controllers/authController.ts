...
export async function registerUser(req: Request, res: Response) {
    ...
    try {
        ...
        // create a user object and save in th db
...
/**
 * Validate user info (email, password)
 * @param data 
 */
...
export async function loginUser(req: Request, res: Response) {
    ...
    try {
        ...
        const token: string = jwt.sign(
            ...
            // attached the token and send it back to the client
    ...
    /**
 * Middelware logic to verify the client JWT token
 * @param req
 * @param res
 * @param next
 */
...
export function verifyToken(req: Request, res: Response, next: NextFunction) {
    ...
    if(!token){
        res.status(400).json({ error: "Access denined."});
        return; 
    }
    try {
        if (token)
